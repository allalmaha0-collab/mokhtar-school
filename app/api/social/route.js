...
export async function GET() {
  const posts = await prisma.socialPost.findMany({ orderBy: { date: 'desc' } });
...
export async function POST(req) {
  ...
  const post = await prisma.socialPost.create({
    data: { title: body.title, content: body.content, category: body.category || 'توعية' },
  });
...
export async function DELETE(req) {
  ...
  if (body.id) {
    await prisma.socialPost.delete({ where: { id: body.id } });
  } else {
