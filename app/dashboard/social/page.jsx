...
import { Plus, Trash2, Edit2, Trash, Heart, Save, X, ArrowRight } from 'lucide-react';
import toast from 'react-hot-toast';

const CATEGORIES = ['توعية', 'إرشاد', 'دعم نفسي', 'أنشطة', 'إعلان'];
...
export default function DashboardSocialPage() {
  ...
  const [form,     setForm]     = useState({ title: '', content: '', category: 'توعية' });
  ...
  function openNew() { setForm({ title: '', content: '', category: 'توعية' }); setEditing(null); setShowForm(true); }
  ...
  async function remove(id) {
    ...
    const res = await fetch(`/api/social`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) });
    if (res.ok) { toast.success('تم الحذف'); load(); }
    else toast.error('خطأ في الحذف');
  }

  async function deleteAll() {
    if (!confirm(`⚠️ حذف جميع منشورات المختص (${posts.length})؟`)) return;
    setDeleting(true);
    // Delete one by one since we don't have deleteMany endpoint
    for (const p of posts) {
      await fetch('/api/social', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: p.id }) });
    }
    toast.success('تم حذف الكل'); load();
    setDeleting(false);
  }

  const catColors = { 'توعية': 'bg-blue-100 text-blue-700', 'إرشاد': 'bg-green-100 text-green-700', 'دعم نفسي': 'bg-purple-100 text-purple-700', 'أنشطة': 'bg-orange-100 text-orange-700', 'إعلان': 'bg-red-100 text-red-700' };

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-black text-primary dark:text-white">المختص الاجتماعي</h1>
          <p className="text-gray-500 dark:text-gray-400 text-sm">{posts.length} منشور — توعية وإرشاد التلاميذ</p>
        </div>
        ...
        <div className="card p-6 border-2 border-primary/20">
          ...
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <div>
              ...
              <textarea className="input resize-none h-32" placeholder="محتوى المنشور التوعوي أو الإرشادي..." value={form.content} onChange={e => setForm(p => ({ ...p, content: e.target.value }))} required />
            </div>
            <div>
      ...
      {loading ? (
        ...
        <div className="card p-16 text-center text-gray-400">
          ...
          <p className="mb-4 font-bold">لا توجد منشورات</p>
