...
import { Plus, Trash2, Edit2, Trash, FileText, Save, X, ExternalLink, GraduationCap } from 'lucide-react';
...
const catColors = {
  'نماذج امتحانات':     'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300',
  'روائز التقييم':      'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300',
  'ملفات الدعم التربوي':'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300',
  'بطاقات التتبع':      'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300',
  'وثائق إدارية':       'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300',
  'مذكرات وتقارير':     'bg-teal-100 text-teal-700 dark:bg-teal-900/30 dark:text-teal-300',
};
...
export default function StudentDocsPage() {
  ...
  const [form, setForm] = useState({ title: '', description: '', fileUrl: '', category: 'نماذج امتحانات', type: 'student', order: 0 });

  useEffect(() => { load(); }, []);

  async function load() {
    setLoading(true);
    const res = await fetch('/api/resources?type=student');
    const data = await res.json();
    setResources(data.resources || []);
    setLoading(false);
  }

  function openNew()   { setForm({ title: '', description: '', fileUrl: '', category: 'نماذج امتحانات', type: 'student', order: 0 }); setEditing(null); setShowForm(true); }
  function openEdit(r) { setForm({ title: r.title, description: r.description || '', fileUrl: r.fileUrl || '', category: r.category, type: 'student', order: r.order }); setEditing(r.id); setShowForm(true); }
  ...
  async function deleteAll() {
    if (!confirm(`⚠️ حذف جميع ملفات التلاميذ (${resources.length})؟`)) return;
    const res = await fetch('/api/resources', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'student' }) });
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-black text-primary dark:text-white flex items-center gap-2">
            <GraduationCap size={24} /> وثائق التلاميذ
          </h1>
          <p className="text-gray-500 dark:text-gray-400 text-sm">
            {resources.length} وثيقة — نماذج امتحانات، روائز، وثائق تربوية
          </p>
        </div>
          ...
          <form onSubmit={handleSave} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                ...
                <input className="input" placeholder="عنوان الوثيقة أو النموذج..." value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} required />
              </div>
              <div>
              ...
              <div className="col-span-2">
                ...
                <textarea className="input resize-none h-20" placeholder="وصف مختصر للوثيقة..." value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} />
              </div>
            </div>
                  ...
                  <div key={r.id} className="card p-4 hover:shadow-md transition-all">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      ...
                      <div className="flex gap-1 flex-shrink-0">
                        <button onClick={() => openEdit(r)} className="p-1.5 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20 text-blue-500">
                          <Edit2 size={13} />
                        </button>
                        <button onClick={() => remove(r.id)} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 text-red-400">
                          <Trash2 size={13} />
                        </button>
                      </div>
                    </div>
                    ...
                    {r.fileUrl && (
                      <a href={r.fileUrl} target="_blank" rel="noopener noreferrer"
                        className="flex items-center gap-1.5 text-primary hover:underline text-xs font-bold">
